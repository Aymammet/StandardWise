...
- [ ] Staging create-account flow creates the Firebase Auth account and Firestore user profile.
- [ ] Staging create-account flow rejects duplicate email addresses with a friendly message.
- [ ] Staging newly registered regular user can access the app immediately.
- [ ] Staging existing unverified Firebase users can sign in.
...
- [ ] Registration no longer requires email activation in Staging.
- [ ] Gmail test: record whether password reset email lands in Inbox, Spam, or Promotions.
- [ ] iCloud test: record whether password reset emails land in Inbox or Junk.
- [ ] Outlook/Hotmail test: record whether password reset emails land in Inbox or Junk.
- [ ] School email test, if available: record whether password reset emails arrive or are filtered.
